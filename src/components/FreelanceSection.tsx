...
import {
  ...
  ShoppingCart, LayoutGrid, Search, Compass, PenTool,
} from 'lucide-react';
...
// Archivé le 2026-09-13 : les réalisations WebLocal sont désormais importées
// directement dans la section Projets, ce panneau devenait redondant (et
// évoquait trop une vitrine d'agence). Code conservé intact, juste désactivé —
// repasser à true pour le réafficher.
const SHOW_WEBLOCAL_PANEL = false;
...
/** Étape de la méthode, reliée par un connecteur vertical (sauf la dernière). */
const ProcessStep = ({
  index,
  icon: Icon,
  title,
  ...
  >
    <div className="relative flex shrink-0 flex-col items-center">
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand/10 text-brand ring-1 ring-brand/25">
        <Icon size={20} />
      </div>
...
/** Maquette de navigateur avec la vraie capture d'écran du site de l'agence. (archivé avec SHOW_WEBLOCAL_PANEL) */
...
const FreelanceSection = () => {
  ...
  // Archivé avec SHOW_WEBLOCAL_PANEL — conservé pour une réactivation future.
  const services = [
  ...
  const process = [
    { icon: Compass, titleKey: 'freelance_process_1_title', descKey: 'freelance_process_1_desc' },
    { icon: PenTool, titleKey: 'freelance_process_2_title', descKey: 'freelance_process_2_desc' },
    { icon: Zap, titleKey: 'freelance_process_3_title', descKey: 'freelance_process_3_desc' },
  ...
  return (
    <section id="methode" className="section-padding relative overflow-hidden">
      <div className="container relative z-10 mx-auto px-6">
                  ...
                  <div className="relative grid items-center gap-10 lg:grid-cols-2">
                    <div>
                      ...
                      <h2 className="font-display text-4xl font-bold tracking-tight md:text-5xl">
                        Projets de <span className="text-gradient-animated">développement web</span>
                      </h2>
        ...
        >
          <div className="relative">
            {/* Halo animé conservé (même dégradé teal → cyan → indigo, en rotation continue) */}
            <div className="pointer-events-none absolute -inset-3 rounded-[46px] bg-[conic-gradient(from_0deg,hsl(var(--brand)),hsl(var(--brand-2)),hsl(var(--brand-3)),hsl(var(--brand)))] opacity-40 blur-2xl motion-safe:animate-spin-slow" />
            <ShineBorder color={SHINE_COLORS} borderWidth={2.5} duration={9} borderRadius={40}>
              <div className="relative overflow-hidden rounded-[2.5rem] glass p-8 md:p-14">
                <div className="pointer-events-none absolute -left-20 -top-20 h-72 w-72 rounded-full bg-brand/15 blur-[110px]" />
